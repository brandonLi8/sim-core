...
/**
 * Unit test file for `SIM_CORE/util/Range`. Run `npm coverage` to see test coverage results.
 *
...
define( require => {
  ...
  return () => {
    ...
    // basic
    truenit.ok( A().setMin( 5 ).min === 5, 'setMin' );
    truenit.ok( A().setMax( 5 ).max === 5, 'setMax' );
    truenit.ok( A().setMinMax( 5, 5 ).max === 5, 'setMinMax' );
    truenit.ok( A().setMinMax( 5, 5 ).min === 5, 'setMinMax' );

    // dilate/erode
    truenit.ok( A().dilate( 1.5 ).equals( new Range( -0.5, 3.5 ) ), 'dilate' );
    truenit.ok( A().erode( 0.5 ).equals( new Range( 1.5, 1.5 ) ), 'erode' );

    // expand
    truenit.ok( A().expand( 0, 0 ).equals( A() ), 'expand' );
    truenit.ok( A().expand( 1, -2 ).equals( Range.ZERO ), 'expand' );
    truenit.ok( A().expand( -1, 2 ).equals( new Range( 2, 4 ) ), 'expand' );

    // shift
    truenit.ok( A().shift( 0 ).equals( A() ), 'shift' );
    truenit.ok( A().shift( 5 ).equals( new Range( 6, 7 ) ), 'shift' );
  };
} );