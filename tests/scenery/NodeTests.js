...
define( require => {
  ...
  return () => {
    ...
    // Test 7: Max width / max height
    const F = new Node( { left: 5, top: 4, width: 4, height: 8, maxWidth: 1 } );

    truenit.ok( F.parentBounds.equals( new Bounds( 5, 4, 6, 6 ) ) );

    F.maxHeight = 4;
    console.log (F.parentBounds.toString())
    truenit.ok( F.parentBounds.equals( new Bounds( 5, 4, 5.5, 8 ) ) );
  };
} );